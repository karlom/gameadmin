...
<body>
<div id="position">
<b><{$lang->menu->class->userInfo}>：<{$lang->menu->color}></b>
</div>
...
<table class="DataGrid" cellspacing="0" style="margin:5px;">
	<tr>
		...
		<th><{$lang->color->equipUid}></th>
		<th><{$lang->color->equipId}></th>
		<th><{$lang->color->stoneId}></th>
		<th><{$lang->color->stoneNum}></th>
		<th><{$lang->color->cost}></th>
		<th><{$lang->color->preColor}></th>
		<th><{$lang->color->afterColor}></th>
	</tr>
	<{foreach name=loop from=$viewData item=item key=key}>
		<tr class='<{cycle values="trEven,trOdd"}>'>
			...
			<td><{$item.level}>&nbsp;</td>
			<td><{$item.equip_uid}>&nbsp;</td>
			<td><{$arrItemsAll[$item.equip_id].name}>&nbsp;</td>
			<td><{$arrItemsAll[$item.stone_id].name}>&nbsp;</td>
			<td><{$item.stone_num}>&nbsp;</td>
			<td><{$item.cost}>&nbsp;</td>
			<td><{$item.pre_color}>&nbsp;</td>
			<td><{$item.after_color}>&nbsp;</td>
		</tr>
	<{/foreach}>
</table>
</form>
