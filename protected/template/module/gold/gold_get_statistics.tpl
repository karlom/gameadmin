...
<title><{$lang->menu->goldGetAnalyse}></title>
</head>
...
<div id="position"><{$lang->menu->class->spendData}>：<{$lang->menu->goldGetAnalyse}></div>
...
<{ if $gold_decrease_statistics || $gold_increase_statistics}>
	<table class="DataGrid sortable table_list" cellspacing="0" style="margin-bottom:20px;">
		<caption class="table_list_head">
			...
			<{$lang->gold->getOfGold}>
			<font style="color:#f00;">(<{$lang->gold->gold_increase}>)</font>
		...
		<tr class="table_list_head" align="center">
			...
			<th width="100" class="col_gold"><{$lang->gold->totalGetGold}></th>
			<th width="80" ><{$lang->page->percentage}></th>
			<th width="100" class="col_liquan"><{$lang->gold->totalGetLiquan}></th>
			<th width="80" ><{$lang->page->percentage}></th>
			<th width="100" class="col_gold_op_count"><{$lang->gold->goldOpCount}></th>
			<th width="100" class="col_liquan_op_count"><{$lang->gold->liquanOpCount}></th>
		</tr>
		</thead>
		<{ if $gold_decrease_statistics }>
		<tbody>
			<!--  Start  元宝消耗-->
			<{assign var=count_of_gold value=0}>
			<{assign var=count_of_liquan value=0}>
			<{assign var=count_of_gold_op value=0}>
			<{assign var=count_of_liquan_op value=0}>
			<{assign var=rowspan_1 value=$gold_decrease_statistics|@count}>
			
			<{foreach from=$gold_decrease_statistics item=log name=gold_decrease_statistics_loop }>
		    <tr id="<{$log.type}>" align="center" class="row <{cycle values="trEven,trOdd"}> <{$log.type}>">

				<{assign var=count_of_gold value=$count_of_gold+$log.all_gold}>
				<{assign var=count_of_liquan value=$count_of_liquan+$log.all_liquan}>
				<{assign var=count_of_gold_op value=$count_of_gold_op+$log.gold_op_count}>
				<{assign var=count_of_liquan_op value=$count_of_liquan_op+$log.liquan_op_count}>
				<td><{ $log.type }>&nbsp;</td>
				<td><{ if $op_type[$log.type] }><{ $op_type[$log.type] }><{else}>-<{/if}>&nbsp;</td>
				...
				<td class="col_gold_op_count"><{ if $log.gold_op_count}><{ $log.gold_op_count}><{else}>0<{/if}>&nbsp;</td>
		        <td class="col_liquan_op_count"><{ if $log.liquan_op_count}><{$log.liquan_op_count}><{else}>0<{/if}>&nbsp;</td>
			</tr>
			<{/foreach}>
		</tbody>
		<tfoot>
			<tr align="center" <{ if $smarty.foreach.gold_decrease_statistics_loop.index is even }> class="odd"<{ /if }> >
				...
				<td></td>
				<td class="col_liquan"><{$count_of_liquan}></td>
				<td></td>
				<td class="col_gold_op_count"><{$count_of_gold_op}></td>
				<td class="col_liquan_op_count"><{$count_of_liquan_op}></td>
			</tr>
		</tfoot>
			<!--  End  元宝消耗-->
		<{else}>
			<tr><td colspan="6"><{$lang->page->noData}></td></tr>
		<{/if}>
