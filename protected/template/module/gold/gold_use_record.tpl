...
<title><{$lang->menu->goldUseRecord}></title>

<body>
<div id="position">
<b><{$lang->menu->class->payAndSpand}>：<{$lang->menu->goldUseRecord}></b>
</div>
...
<table class="DataGrid" cellspacing="0" style="margin:5px;">
	<tr>
		...
		<th><{$lang->gold->consume}><{$lang->currency->yuanbao}><{$lang->page->number}></th>
		<th><{$lang->gold->consume}><{$lang->page->time}></th>
		<th><{$lang->gold->op_type}></th>
		<th><{$lang->item->itemID}> </th>
		<th> <{$lang->gold->item_name}></th>
	...
	<{foreach name=loop from=$viewData item=item key=key}>
		<tr class='<{cycle values="trEven,trOdd"}>' align="center">
			...
			<td><{$item.level}>&nbsp;</td>
			<td <{if 2 == $item.type}>class="red"<{/if}>><{if 2 == $item.type}>-<{/if}><{$item.gold}>&nbsp;</td>
