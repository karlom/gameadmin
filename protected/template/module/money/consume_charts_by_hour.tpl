...
<title><{$lang->menu->moneyConsumeChartsByHour}></title>
...
<div id="position"><{$lang->menu->class->spendData}>：<{$lang->menu->moneyConsumeChartsByHour}></div>
	...
	<form action="?action=search" id="frm" method="get"  style="display:inline;">
		<table cellspacing="1" cellpadding="5" class="SumDataGrid" width="900">
			<tr>
				...
				<td>
					<{$lang->money->money_type}>：<{html_options options=$dictMoneyType selected=$selectMoneyType name='select_money_type'}>
				</td>
				<td>
					<{$lang->money->consumption_type}>:<input id='consumption_type_widget' name='consumption_type_widget' type="text" size='30' value='<{if $selectConsumptionType > 0}><{$selectConsumptionType}> | <{$consumptionType[$selectConsumptionType]}><{/if}>' /> 
...
<table class="SumDataGrid" style="min-width:<{$minWidth}>px;">
	<tr align="center">
		<th colspan="<{math equation="x+1" x=$countOfDays}>">
			...
			[<span style="color:#f00;"><{$dictMoneyType[$selectMoneyType]}></span>]<{$lang->money->dailyConsumptionGraph}>  
			<{$lang->money->consumption_type}> ： [<span style="color:#f00;"><{if $selectConsumptionType > 0}><{ $consumptionType[$selectConsumptionType]}><{else}><{$lang->page->unlimited}><{/if}></span>]
		</th>
	</tr>
	<tr align="center" valign="bottom"  >
		<th valign="middle" width="150"><{$lang->money->population}></th>
	...
	<tr align="center" valign="bottom"  >
		<th valign="middle"><{$lang->money->money_decrease}></th>
		<{assign var=division value=$top_money_count*$highlightPercentage}>
		...
		<td>
			...
			<hr class="<{$class}>" style=" height:<{math equation="(x/y)*120" x=$log.cost y=$top_money_count}>px;" />
		</td>
		<{/foreach}>
	</tr>
	<tr align="center">
...
<table class="SumDataGrid" style="min-width:<{$minWidth}>px;">
	<tr align="center">
		<th colspan="25">
			<{$startTime|date_format:"%Y-%m-%d"}> - <{$endTime|date_format:"%Y-%m-%d"}>  
			[<span style="color:#f00;"><{$dictMoneyType[$selectMoneyType]}></span>]<{$lang->money->hourConsumptionGraph}>  
			<{$lang->money->consumption_type}> ： [<span style="color:#f00;"><{if $selectConsumptionType > 0}><{ $consumptionType[$selectConsumptionType]}><{else}><{$lang->page->unlimited}><{/if}></span>]
		</th>
	</tr>
	<tr align="center" valign="bottom">
		<th valign="middle" width="150"><{$lang->money->money_decrease}></th>
...
<table class="SumDataGrid" style="min-width:<{$minWidth}>px;">
	<tr align="center">
		<th colspan="26">
			<{$startTime|date_format:"%Y-%m-%d"}> - <{$endTime|date_format:"%Y-%m-%d"}>  
			[<span style="color:#f00;"><{$dictMoneyType[$selectMoneyType]}></span>]<{$lang->money->dailyHourConsumptionGraph}>  
			<{$lang->money->consumption_type}> ： [<span style="color:#f00;"><{if $selectConsumptionType > 0}><{ $consumptionType[$selectConsumptionType]}><{else}><{$lang->page->unlimited}><{/if}></span>]
		</th>
	</tr>
	...
	<tr align="center" valign="bottom"  height="148" <{ if $smarty.foreach.logByDayHourResult_loop.index is odd }> class="odd"<{ /if }>>
		...
		<th valign="middle" width="64" ><{$lang->money->money_decrease}></th>
