...
<body>
<div id="position">
<b><{$lang->menu->class->payAndSpand}>：<{$lang->menu->moneyUseRecord}></b>
</div>
...
*}>
	...
	<font color="red"><b><{$lang->money->moneyFlowCount}>: <{$flowGold}><{$lang->money->money}></b></font></br>
...
<table class="DataGrid" cellspacing="0" style="margin:5px;">
	<tr>
		...
		<th><{$lang->money->consume}><{$lang->money->money}><{$lang->page->itemsNum}></th>
		<th><{$lang->money->consume}><{$lang->money->bind_money}><{$lang->page->itemsNum}></th>
		<th><{$lang->money->consume}><{$lang->page->time}></th>
		<th><{$lang->money->op_type}></th>
		<th><{$lang->item->itemID}> | <{$lang->money->item_name}></th>
		<th><{$lang->page->itemsNum}></th>
		<th><{$lang->money->moneyBalance}></th>
		<th><{$lang->money->bind_moneyBalance}></th>
	</tr>
	<{foreach name=loop from=$viewData item=item key=key}>
		<tr class='<{cycle values="trEven,trOdd"}>'>
			...
			<td><{$item.level}>&nbsp;</td>
			<td <{if 2 == $item.mtype}>class="red"<{/if}>><{if 2 == $item.mtype}>-<{/if}><{$item.money}>&nbsp;</td>
			<td <{if 2 == $item.mtype}>class="red"<{/if}>><{if 2 == $item.mtype}>-<{/if}><{$item.bind_money}>&nbsp;</td>
