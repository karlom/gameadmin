...
<title><{$lang->menu->friendChat}></title>
...
<form action="?action=search" id="frm" method="GET" >
	<table cellspacing="1" cellpadding="5" class="SumDataGrid" width="auto">
		<tr>
			...
			<{*
			<td align="right"><{$lang->page->accountName}>:
				<input type="text" name="account_name" id="account_name" size="15" value="<{$account_name}>" />
			</td>
			*}>
			<td align="right"><{$lang->page->roleName}>:
				<input type="text" name="role_name" id="role_name" size="15" value="<{$role_name}>" />
			</td>
...
<div class="main-header">
	<{$lang->page->currentLooking}> <{$lookingDay}> <{if $role_name }>【<{$role_name}>】<{else }><font color="#BB2222"><{$lang->verify->all}></font><{/if}> <{$lang->page->chatLogOf }>
</div>
<{ if $chat_log_list }>
	<!--  Start  聊天日志信息-->
	<div id="chatlog-container">
		<{foreach from=$chat_log_list item=chat_log name=chat_log_loop}>
			<div class="log">
				<span class="time">[<{$chat_log.mtime|date_format:'%H:%M:%S'}>]</span> <span class="name"><{ $chat_log.role_name}> -> <{$chat_log.targetName}></span> : <{ $chat_log.content}><br />
			</div>
		<{/foreach}>
	</div>
	<!--  End  聊天日志信息-->
