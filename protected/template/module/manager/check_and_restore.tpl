...
<script language="javascript">
	$(document).ready(function(){
		...
		$("#btnRestore").click(function(){
			if($("input:checkbox[name='selectItem[]']:checked").size() <1 ){
				alert("<{$lang->msg->selectAtLeastOne}>");
			} else {
...
<form action="?action=search" id="frm" method="post" >
	<table cellspacing="1" cellpadding="5" class="SumDataGrid" width="800">
		<tr>
			...
			*}>
			<input type="hidden" name="role[role_name]" id="role_name" value="<{$role.role_name}>" />
			<input type="hidden" name="currentRoleName" id="currentRoleName" value="<{$currentRoleName}>" />
	...
	<br />
	<div class="red">&nbsp;&nbsp;<{$lang->msg->oldDataHere}></div>
	<br />
	<{if 1 == $onlineStatus}>
	...
	<{if $role.role_name && $role.account_name && $restoreArry && !$onlineStatus}>
	<table class="DataGrid" cellspacing="0">
	
	
		<input id="selectAll" type="checkbox" value="selectAll" name="selectAll" <{if $selectAll }>checked="checked"<{/if}>></input><{$lang->page->selectAll}>
		<br />
		<{foreach from=$restoreArry item=item key=key name=sel}>
			<{if ($smarty.foreach.sel.index+1)%5==1}>
			<tr>
			<{/if}>
			
			<td><input type="checkbox" class="lbCheckbox" name="selectItem[]" value="<{$key}>" <{if @in_array($key,$selectItem) }>checked="checked"<{/if}>/><{$item}></td>
			
			<{if ($smarty.foreach.sel.index+1)%5==0}>
			</tr>
			<{/if}>
		<{/foreach}>

		<tr>
			<td colspan="5"><input type="checkbox" name="newUser" id="newUser" value="yes" <{if $newUser == 'yes' }>checked="checked"<{/if}>>新注册用户选项</input></td>
	    </tr>
		<tr>
			<th colspan="5"><input type="button" name="btnRestore" id="btnRestore" value="<{$lang->page->restoreData}>" /></th>
	    </tr>
	</table>
	<br />
	<{/if}>
...
<{if $role}>
<h1 class="red" align="center">以下是历史数据：</h1>
<br>
<!--  Start  账号信息-->
