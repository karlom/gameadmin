...
<!--  
<div id="position">
<b><{$lang->menu->class->userInfo}>：<{$lang->menu->ship}></b>
</div> 
-->
	...
	<tr style="display:none">
		...
		<table cellspacing="1" cellpadding="3" border="0" class='DataGrid table_list' >
			<tr class='table_list_head'>
		        <th width="10%"><{$lang->player->roleName}></th>
		        <th width="10%"><{$lang->player->accountName}></th>
		        <th width="10%"><{$lang->player->beginTime}></th>
		        <th width="10%"><{$lang->player->endTime}></th>
				<th width="5%"><{$lang->player->isRefresh}></th>
				<th width="5%"><{$lang->player->refreshTimes}></th>
				<th width="25%"><{$lang->player->shipDetail}></th>
				<th width="10%"><{$lang->player->shipTimes}></th>
		        <th width="5%"><{$lang->player->shipColor}></th>
		        <th width="10%"><{$lang->player->shipResult}></th>
			</tr>
		...
		<{foreach name=loop from=$itemT.data item=item}>
			<tr class='<{cycle values="trEven,trOdd"}>'>
				...
				<td><{$item.detail}></td>
				<td align="center"><{$item.ship_times+1}></td>
				<td><{$dictColor[$item.ship_type]}></td>
				<td align="center"><{$dictShipResult[$item.result]}></td>
			</tr>
		<{/foreach}>
		</table>
		</td>
