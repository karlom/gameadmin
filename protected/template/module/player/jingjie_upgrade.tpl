...
<!-- 
<div id="position">
<b><{$lang->menu->class->userInfo}>：<{$lang->menu->jingJieUpgrade}></b>
</div> 
-->
<!-- Start 成功信息提示 -->
<{if $successMsg}>
...
<table style="width: 50%;" cellspacing="1" cellpadding="3" border="0" class="DataGrid table_list" >
	<tr class='table_list_head'>
		...
		<td width="30%"><{$lang->page->detailDeal}></td>
...
<{foreach name=loop from=$viewData.data item=item}>
	<tr class='<{cycle values="trEven,trOdd"}>'>
		...
		<td><{$item.level}></td>
		<td><{$item.jingjie_name}>:<{$numUper[$item.subLv]}></td>
