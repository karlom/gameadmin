...
<div class='divOperation' style="margin-top: 10px;">
	<{$lang->page->inputIPOrRolename}>:<input type="text" id="term" name="term" size="15" value="<{if $term}><{$term}><{/if}>" />
	<input type="submit" name="searchAccount" value="<{$lang->sys->search}>"/>
</div>
</form>
<{if $userList}>
<div class="label">
	<{$lang->player->userList}>:
</div>
<table cellspacing="1" cellpadding="3" border="0" class="table_list">
	...
	<{ foreach from=$userList item=row key=key }>
	...
	<tr class='<{cycle values="trEven,trOdd"}>'> 
		...
		<td align="center"><{$row.total_pay}></td>
		<td align="center"><span class="label" style="color: <{if 1 == $row.online}>green<{else}>gray<{/if}>">
			<{if 1 == $row.online}>
				<{$lang->player->online}> 
			<{else}> 
				<{$lang->player->offline}>
        ...
        <td align="center">
        	...
        	<input type="submit" name="banAccount" value="<{$lang->player->ban}>" />
		</td>
	</tr>
	</form>
	<{/foreach}>
...
<table cellspacing="1" cellpadding="3" border="0" class="table_list">
	...
	<tr class='<{cycle values="trEven,trOdd"}>'> 
		...
		<td><{$row.op_user}></td>
        <td><{if 0 == $row.status || 2 == $row.status}>已解封<{else}><a href="ban_account.php?action=unban&amp;role_name=<{$row.role_name}>;&amp;account_name=<{$row.account_name}>&amp;banTime=0&amp;id=<{$row.id}>" class="link-button">解封</a><{/if}></td>
	</tr>
	<{/foreach}>
</table>
</body>
