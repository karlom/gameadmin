...
<!-- 
<div id="position">
<b><{$lang->menu->class->userInfo}>：<{$lang->menu->wuhunMerge}></b>
</div> 
-->
<!-- Start 成功信息提示 -->
<{if $successMsg}>
...
<table style="width: 50%;" cellspacing="1" cellpadding="3" border="0" class="DataGrid table_list" >
	<tr class='table_list_head'>
		...
		<td width="15%"><{$lang->wuhun->merge}></td>
		<td width="15%"><{$lang->wuhun->beMerged}></td>
	...
	<tr class='<{cycle values="trEven,trOdd"}>'>
		...
		<td><{$item.level}></td>
		<td><span style="color: <{$dictColorValue[$item.target_color]}>"><{$item.target_name}></span> lv:<{$item.target_lv}></td>
		<td><span style="color: <{$dictColorValue[$item.src_color]}>"><{$item.src_name}></span> lv:<{$item.src_lv}></td>
