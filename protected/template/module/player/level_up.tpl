...
<title><{$lang->menu->levelupHitory}></title>

</head>

<body>
<div id="position"><{$lang->menu->class->userInfo}>：<{$lang->menu->levelupHitory}></div>
...
<{ if $level_up_log_list }>
	...
	<table class="DataGrid sortable table_list" cellspacing="0" style="margin-bottom:20px;">
		...
		<tr>
	        ...
	        <th width="25%"><{$lang->page->time}></th>
		</tr>
	
			<{foreach from=$level_up_log_list item=level_up_log name=level_up_log_loop}>
		    <tr align="center" <{ if $smarty.foreach.level_up_log_loop.index is odd }> class="odd"<{ /if }>>
		
				<td><{ $level_up_log.account_name}>&nbsp;</td>
				<td><{ $level_up_log.role_name}>&nbsp;</td>
				<td><{ $level_up_log.prev_level}>&nbsp;</td>
		        <td><{ $level_up_log.level}>&nbsp;</td>
				<td><a href="javascript:void(0)" class="show-ip"><{ $level_up_log.ip}></a>&nbsp;</td>
		        <td><{ $level_up_log.mtime|date_format:'%Y-%m-%d %H:%M:%S'}>&nbsp;</td>
			</tr>
			<{/foreach}>
	</table>
	<!--  End  登录信息-->
