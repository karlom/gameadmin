<?php
/**
 * upload_shop_consume_bind.php
 * 运行方式：php upload_shop_consume_bind.php [startDate [endDate [server]]] 
...
$timeFile = "ts_shop_consume_bind_time.txt";
$timeFileFullPath = TS_LOG_DIR."/".$timeFile;
$toWrite = true;

$logFileName = "log_shop_consume_bind_{$day}.log";
$errLogName = "err_log_shop_consume_bind_{$day}.log";
...
if(!empty($loadServerList)){
	...
	foreach($loadServerList as $k => $v){
		...
		$sqlCnt = "select count(*) as cnt from " . $table . " where mtime>={$startTime} and mtime<{$endTime} and item_id>0 and liquan<0 and type in (50028,50029,50076,50105,50106) ";
		...
		$sql = " select t1.*, t2.pf, t2.ip from " .
				...
				" IF( LENGTH(account_name)>32, substring(account_name, 7), account_name) as openid, role_name, level, item_id, num, liquan, remain_liquan " .
				" from " . $table . " where mtime>={$startTime} and mtime<{$endTime} and item_id>0 and liquan<0 and type in (50028,50029,50076,50105,50106) ) t1," .
		...
		if(!empty($ret)) {
			foreach($ret as $j => $w){
				...
				$tsLogArray = array(
					...
					'f_goods_price' => -round($w['liquan']/$w['num']),
					'f_goods_num' => $w['num'],
					'f_consume_yuanbao_num' => -$w['liquan'],
					'f_overplus_yuanbao_num' => $w['remain_liquan'],
					'f_yuanbao_channel' => "2",	//2=绑定仙石
				);
