...
/**
 * upload_level_up.php
 * 运行方式：php upload_level_up.php [startDate [endDate [server]]] 
...
$timeFile = "ts_level_up_time.txt";
$timeFileFullPath = TS_LOG_DIR."/".$timeFile;
$toWrite = true;

$logFileName = "log_level_up_{$day}.log";
$errLogName = "err_log_level_up_{$day}.log";
...
if(!empty($loadServerList)){
	...
	foreach($loadServerList as $k => $v){
		...
		$sql = " select t1.*, t2.pf from " .
				"(select id, mtime, mdate, uuid, IF( LENGTH(account_name)>32, substring(account_name, 1, 5), '0') as sid, " .
				" IF( LENGTH(account_name)>32, substring(account_name, 7), account_name) as openid, role_name, ip, level " .
				" from " . $table . " where mtime>={$startTime} and mtime<{$endTime} ) t1," .
				" ( select a1.uuid, a1.pf, a1.mtime from t_log_login a1, (select uuid, max(mtime) as mtime from t_log_login group by uuid) a2 " .
