...
/**
 * upload_login.php
 * 运行方式：php upload_login.php [startDate [endDate [server]]] 
...
$timeFile = "ts_login_time.txt";
$timeFileFullPath = TS_LOG_DIR."/".$timeFile;
$toWrite = true;

$logFileName = "log_login_{$day}.log";
$errLogName = "err_log_login_{$day}.log";
...
if(!empty($loadServerList)){
	...
	foreach($loadServerList as $k => $v){
		...
		$sql = "select id, pf, mtime, mdate, IF( LENGTH(account_name)>32, substring(account_name, 1, 5), '0') as sid, " .
				" IF( LENGTH(account_name)>32, substring(account_name, 7), account_name) as openid, role_name, ip, level, xianzun, zhandouli " .
				" from " . $table . " where mtime>={$startTime} and mtime<{$endTime} order by mtime ";
		...
		if(!empty($ret)) {
			foreach($ret as $j => $w){
				...
				$tsLogArray = array(
					...
					'f_character_ip' => $w['ip'],
					'f_vip_grade' => $w['xianzun'],
					'f_fighting' => $w['zhandouli'],
				);
