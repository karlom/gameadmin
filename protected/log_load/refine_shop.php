	...
	if($loadServerList){
    	...
    	foreach ($loadServerList as $k => $v){
    		try{
    			...
    			$sql = 'UPDATE ' . T_REFINE_SHOP . ' SET status = ' . CS_FINISHED . ' WHERE status = ' . CS_IN_PROGRESS . ' AND date_type = 0 AND end_time <= ' . $time;
    			GFetchRowOne($sql);
    			
    			// 更新状态为进行中且结束时间已经过去了的配置为已结束(按开服日期)。
    			$sql = 'UPDATE ' . T_REFINE_SHOP . ' SET status = ' . CS_FINISHED . ' WHERE status = ' . CS_IN_PROGRESS . ' AND date_type = 1 AND end_open_day_time <= ' . $sinceOnlinedateTimestamp;
    			GFetchRowOne($sql);
    		
        		$inProgress = false;
    			// 查询指定时间范围是否存在正在进行的活动(确切日期)
    			$sql = "SELECT count(*) in_progress_count FROM ".T_REFINE_SHOP." WHERE start_time <= $time AND end_time >= $time AND date_type = 0 AND status = " . CS_IN_PROGRESS;
    			...
    			else
    			{
    				...
    				$sql = "SELECT count(*) in_progress_count FROM ".T_REFINE_SHOP." WHERE start_open_day_time <= $sinceOnlinedateTimestamp AND end_open_day_time >= $sinceOnlinedateTimestamp AND date_type = 1 AND status = " . CS_IN_PROGRESS;
    				$countResult = GFetchRowOne($sql);
    				if ( $countResult['in_progress_count'] > 0 )
    				{
    			...
    			{// 如果指定时间范围内没有进行中的活动，则准备发送
    				...
    				$sql = "SELECT * FROM ".T_REFINE_SHOP." WHERE start_time <= $time AND end_time >= $time AND status = " . CS_WAITING . " AND date_type = 0 ORDER BY start_time ASC LIMIT 1";
    				$result = GFetchRowOne($sql);
    				
    				// 获取等待发送中开始时间最接近的一条配置(按开服日期)。
    				$sql2 = "SELECT * FROM ".T_REFINE_SHOP." WHERE start_open_day_time <= $sinceOnlinedateTimestamp AND end_open_day_time >= $sinceOnlinedateTimestamp AND status = " . CS_WAITING . " AND date_type = 1 ORDER BY start_open_day_time ASC LIMIT 1";
    				...
    				if ( isset($result['id']) )
    				{
    					...
    					$ret = httpRequestResult($k, 'setcheapshop', array('refineshop' => json_encode($params), 'type' => 2 ));
    					echo 'response: ' . print_r($ret, true);
    					if($ret['result'] == 1)
    					{
    						//更新配置的状态为正在进行
    						$sql = 'UPDATE ' . T_REFINE_SHOP . ' SET status = ' . CS_IN_PROGRESS . ' WHERE id = ' . $result['id'];
    						GFetchRowOne($sql);
            ...
            if('' != $message){
            	$logfile = "/tmp/refineshop_log_".date("Y-m-d", $time ).".log";
