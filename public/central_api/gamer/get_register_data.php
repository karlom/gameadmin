...
try {
	...
	$sqlCount = "SELECT count(*) c FROM " . T_LOG_REGISTER . " WHERE mtime >= {$beginTime} and mtime < {$endTime}";
	...
	{
		$start = ($index - 1) * $step;
                $sql = "SELECT `year`, `month`,`day`,`hour`,`min`, `mtime`,`uuid`, `account_name`, `role_name`, `ip`, `job`, `sex`, `level` FROM " . T_LOG_REGISTER . " WHERE mtime >= {$beginTime} and mtime < {$endTime} LIMIT $start, $step";
	//	echo $ sql ; 	
