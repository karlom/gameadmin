...
try {
	...
	$sqlCount = "SELECT count(*) c FROM " . $tablename . " WHERE mtime >= {$beginTime} and mtime < {$endTime}";
	$count = GFetchRowOne($sqlCount);

	$count = intval( $count['c'] );
	$times = ceil( $count / $step ) ;
	$index = 1;
        $sql = "SELECT `account_name` FROM " . $tablename . " WHERE mtime >= {$beginTime} and mtime < {$endTime} LIMIT $start, $step";   
	while( $times-- )
	{
		$start = ($index - 1) * $step;
		$sql = "SELECT `account_name` FROM " . $tablename . " WHERE mtime >= {$beginTime} and mtime < {$endTime} LIMIT $start, $step";  	
