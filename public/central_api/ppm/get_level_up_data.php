...
// 搜索指定时间内的升级日志, [$beginTime,$endTime) 防止数据丢失和重复！！
try {
	$step = 1000;//控制每次输出多少条,因为一次输出全部，保存结果的数组太大会造成PHP内存溢出
	$sqlCount = "SELECT count(*) c FROM " . T_LOG_LEVEL_UP . " WHERE mtime >= {$beginTime} and mtime < {$endTime}";
	...
	{
		$start = ($index - 1) * $step;
                $sql = "SELECT `year`, `month`,`day`,`hour`,`min`, `mtime`, `mdate`, `uuid`,  `account_name`, `role_name`, `ip`, `prev_level`, `level` FROM " . T_LOG_LEVEL_UP . " WHERE mtime >= {$beginTime} and mtime < {$endTime} LIMIT $start, $step";
	//	echo $ sql ; 	
