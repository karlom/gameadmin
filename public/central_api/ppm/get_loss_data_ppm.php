...
try {
	...
	$sqlCount = "SELECT count(*) c FROM " . $table . " WHERE mtime >= {$beginTime} and mtime < {$endTime}";
	$count = GFetchRowOne($sqlCount);

	$count = intval( $count['c'] );
	$times = ceil( $count / $step ) ;
	$index = 1;
        $sql = "SELECT `mdate`, `mtime`, `account_name`, `role_name`,  `step`,  `ip`, `year`, `month`, `day`, `hour`, `min` FROM " . $table . " WHERE mtime >= {$beginTime} and mtime < {$endTime} LIMIT $start, $step";   
	while( $times-- )
	{
		$start = ($index - 1) * $step;
		$sql = "SELECT `mdate`, `mtime`, `account_name`, `role_name`,  `step`,  `ip`, `year`, `month`, `day`, `hour`, `min` FROM " . $table . " WHERE mtime >= {$beginTime} and mtime < {$endTime} LIMIT $start, $step";  	
