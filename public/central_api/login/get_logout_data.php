...
try {
	...
	$sqlCount = "SELECT count(*) c FROM " . T_LOG_LOGOUT . " WHERE mtime >= {$beginTime} and mtime < {$endTime}";
	$count = GFetchRowOne($sqlCount);

	$count = intval( $count['c'] );
	$times = ceil( $count / $step ) ;
	$index = 1;
        $sql = "SELECT `year`, `day`, `month`, `day`, `hour`, `mtime`, `mdate`, `uuid`, `account_name`, `role_name`, `level`, `ip`, `online_time` FROM " . T_LOG_LOGOUT . " WHERE mtime >= {$beginTime} and mtime < {$endTime} LIMIT $start, $step";   
	while( $times-- )
	{
		$start = ($index - 1) * $step;
		$sql = "SELECT `year`, `day`, `month`, `day`, `hour`, `mtime`, `mdate`, `uuid`, `account_name`, `role_name`, `level`, `ip`, `online_time` FROM " . T_LOG_LOGOUT . " WHERE mtime >= {$beginTime} and mtime < {$endTime} LIMIT $start, $step";  	
