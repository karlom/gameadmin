...
if ($action == 'ajax') {
    ...
    try {
        $sql = " select count(*) as count_vip  from  t_log_open_vip" ." where mtime>={$beginTime} and mtime<={$endTime}";
...
//		$centralLog->failed(CENTRAL_LOG_QUERY_SQL_ERROR, $e->getMessage());
    ...
    $sql = " select * from `" . t_log_open_vip . "` where `mtime` >= '" . $beginTime . "' and `mtime` <= '" . $endTime . "'";
    try {
