...
$smarty->display("module/rank/wing_rank.tpl");

function getAttackRank($where, $startNum, $record, &$counts) {
	
	$sqlCount = "select count(*) as cnt from " . T_LOG_BILLBOARD_WING . " where {$where} ";
	$ret = GFetchRowOne($sqlCount);
	$counts = $ret['cnt'] ? $ret['cnt'] : 0;
	
	$sql = "select uuid, account_name, role_name, level, zhandouli, wingname, rank from " . T_LOG_BILLBOARD_WING . " where {$where} order by rank limit {$startNum}, {$record}";	
