...
function getAttackRank($where, $startNum, $record, &$counts) {
	...
	$sqlCount = "select count(*) as cnt from t_log_billboard_pet_zhandouli where {$where} ";
	$ret = GFetchRowOne($sqlCount);
	$counts = $ret['cnt'] ? $ret['cnt'] : 0;
	
	$sql = "select uuid, role_name, petLv, zhandouli, rank from t_log_billboard_pet_zhandouli where {$where} order by rank limit {$startNum}, {$record}";	
