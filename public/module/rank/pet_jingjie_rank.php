...
$viewData = getJingjieRank($where, $startNum, $record, $counts);
...
function getJingjieRank($where, $startNum, $record, &$counts) {
	
	$sqlCount = "select count(*) as cnt from t_log_billboard_pet_jingjie where {$where} ";
	$ret = GFetchRowOne($sqlCount);
	$counts = $ret['cnt'] ? $ret['cnt'] : 0;
	
	$sql = "select uuid, role_name, petLv, name1,name2, rank from t_log_billboard_pet_jingjie where {$where} order by rank limit {$startNum}, {$record}";	
