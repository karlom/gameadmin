...
$viewData = getHorseRank($where, $startNum, $record, $counts);
...
$smarty->display("module/rank/horse_rank.tpl");

function getHorseRank($where, $startNum, $record, &$counts) {
	
	$sqlCount = "select count(*) as cnt from t_log_billboard_horse where {$where} ";
	$ret = GFetchRowOne($sqlCount);
	$counts = $ret['cnt'] ? $ret['cnt'] : 0;
	
	$sql = "select uuid, account_name, role_name, horse_lv, zhandouli, rank from t_log_billboard_horse where {$where} order by rank limit {$startNum}, {$record}";	
