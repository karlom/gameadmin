...
$viewData = getLiquanRank($where, $startNum, $record, $counts);
...
$smarty->display("module/rank/liquan_remain_rank.tpl");

function getLiquanRank($where, $startNum, $record, &$counts) {
	
//	$sqlCount = "select count(distinct uuid) as cnt from t_log_liquan  where {$where} ";
	$sqlCount = "SELECT COUNT(DISTINCT t1.uuid) AS cnt " .
			" FROM t_log_liquan t1, (SELECT uuid, MAX(mtime) AS mtime FROM t_log_liquan GROUP BY uuid) t2 " .
			" WHERE t2.mtime = t1.mtime AND t1.uuid=t2.uuid AND t1.remain_liquan>0 ";
	$ret = GFetchRowOne($sqlCount);
	$counts = $ret['cnt'] ? $ret['cnt'] : 0;
	
	$sql = "SELECT t1.mtime, t1.uuid, t1.account_name, t1.role_name, t1.remain_liquan " .
			" FROM t_log_liquan t1 , (SELECT uuid, max(mtime) AS mtime FROM t_log_liquan GROUP BY uuid) t2 " .
			" WHERE t2.mtime = t1.mtime AND t1.uuid=t2.uuid AND t1.remain_liquan>0 " .
			" GROUP BY t1.uuid ORDER BY t1.remain_liquan DESC " .
