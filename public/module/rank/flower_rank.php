...
$viewData = getFlowerRank($where, $startNum, $record, $counts);
...
$smarty->display("module/rank/flower_rank.tpl");

function getFlowerRank($where, $startNum, $record, &$counts) {
	
	$sqlCount = "select count(*) as cnt from t_log_billboard_flower_sum where {$where} ";
	$ret = GFetchRowOne($sqlCount);
	$counts = $ret['cnt'] ? $ret['cnt'] : 0;
	
	$sql = "select uuid, role_name, job, cnt, rank from t_log_billboard_flower_sum where {$where} order by rank limit {$startNum}, {$record}";	
