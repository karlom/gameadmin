...
$viewData = getShenwuRank($where, $startNum, $record, $counts);
...
$smarty->display("module/rank/shenwu_rank.tpl");

function getShenwuRank($where, $startNum, $record, &$counts) {
	
	$sqlCount = "select count(*) as cnt from t_log_billboard_jingji where {$where} ";
	$ret = GFetchRowOne($sqlCount);
	$counts = $ret['cnt'] ? $ret['cnt'] : 0;
	
	$sql = "select uuid, role_name, job, score, rank from t_log_billboard_jingji where {$where} order by rank limit {$startNum}, {$record}";	
