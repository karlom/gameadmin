...
$viewData = getJingjieRank($selectDate);
...
$smarty->display("module/rank/jingjie_rank.tpl");

function getJingjieRank($date) {
	$data = array();
...
//	$where = " mtime < {$dateStartTime} ";
	...
	//今日等级
	$sqlTodayLevel = "select uuid, role_name, max(jingjieID) as level from ". T_LOG_JINGJIE ." where mtime<={$dateEndTime} group by uuid order by level desc limit 50 ";

	//昨日等级
	$sqlYestodayLevel = "select uuid, role_name, max(jingjieID) as level from ". T_LOG_JINGJIE ." where mtime<{$dateStartTime} group by uuid ";
	/*
