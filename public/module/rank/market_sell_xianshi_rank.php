...
$smarty->display("module/rank/market_sell_xianshi_rank.tpl");

function getMarketDealRank($where, $startNum, $record, &$counts) {
	
	$sqlCount = "select count(distinct uuid) as cnt from t_log_gold  where {$where} AND gold>0 AND type=20018 ";
	$ret = GFetchRowOne($sqlCount);
	$counts = $ret['cnt'] ? $ret['cnt'] : 0;
	
	$sql = "select uuid, account_name, role_name, sum(gold) as sum_gold from t_log_gold  where {$where} AND gold>0 AND type=20018 group by uuid order by sum_gold desc limit {$startNum}, {$record}";	
