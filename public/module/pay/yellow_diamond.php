...
$smarty->assign ($data);
$smarty->display("module/pay/yellow_diamond.tpl");
...
function getStatisticsData($where){
	//总
	//开通
	$sql01 = "select count(distinct uuid ) as open_pt_count from " . T_LOG_OPEN_VIP . " where isYear=0 AND yellowDiamond=0";
	$sql02 = "select count(distinct uuid ) as open_year_count from " . T_LOG_OPEN_VIP . " where isYear=1 AND yellowDiamondYear=0";
	$sql01Result = GFetchRowOne($sql01);
	$sql02Result = GFetchRowOne($sql02);
	
	//续费人数
	$sql03 = "select count(distinct uuid ) as renew_pt_role_count from " . T_LOG_OPEN_VIP . " where isYear=0 AND yellowDiamond=1 ";
	$sql04 = "select count(distinct uuid ) as renew_year_role_count from " . T_LOG_OPEN_VIP . " where isYear=1 AND yellowDiamondYear=1 ";
	$sql03Result = GFetchRowOne($sql03);
	$sql04Result = GFetchRowOne($sql04);
	
	//续费次数
	$sql05 = "select count(*) as renew_pt_count from " . T_LOG_OPEN_VIP . " where isYear=0 AND yellowDiamond=1 ";
	$sql06 = "select count(*) as renew_year_count from " . T_LOG_OPEN_VIP . " where isYear=1 AND yellowDiamondYear=1 ";
	$sql05Result = GFetchRowOne($sql05);
	$sql06Result = GFetchRowOne($sql06);

	//最高单日开通次数
	$sql07 = "select mtime,year,month,day,count(*) as count from " . T_LOG_OPEN_VIP . " where (yellowDiamond=0 AND isYear=0) or (yellowDiamondYear=0 AND isYear=1 ) group by year, month, day";
	...
	//最高单日续费次数
	$sql08 = "select mtime,year,month,day,count(*) as count from " . T_LOG_OPEN_VIP . " where (yellowDiamond=1 AND isYear=0) or (yellowDiamondYear=1 AND isYear=1 )  group by year, month, day";
	...
	//最高单人单日续费次数
	$sql09 = "select mtime,year,month,day,uuid, account_name, role_name, count(*) as count from " . T_LOG_OPEN_VIP . " where (yellowDiamond=1 AND isYear=0) or (yellowDiamondYear=1 AND isYear=1 )  group by year, month, day,uuid";
	...
	//日期范围内
	//开通
	$sql11 = "select count(distinct uuid ) as open_pt_count from " . T_LOG_OPEN_VIP . " where {$where} AND isYear=0 AND yellowDiamond=0";
	$sql12 = "select count(distinct uuid ) as open_year_count from " . T_LOG_OPEN_VIP . " where {$where} AND isYear=1 AND yellowDiamondYear=0";
	$sql11Result = GFetchRowOne($sql11);
	$sql12Result = GFetchRowOne($sql12);
	//续费人数
	$sql13 = "select count(distinct uuid ) as renew_pt_role_count from " . T_LOG_OPEN_VIP . " where {$where} AND isYear=0 AND yellowDiamond=1 ";
	$sql14 = "select count(distinct uuid ) as renew_year_role_count from " . T_LOG_OPEN_VIP . " where {$where} AND isYear=1 AND yellowDiamondYear=1 ";
	$sql13Result = GFetchRowOne($sql13);
	$sql14Result = GFetchRowOne($sql14);
	//续费次数
	$sql15 = "select count(*) as renew_pt_count from " . T_LOG_OPEN_VIP . " where {$where} AND isYear=0 AND yellowDiamond=1 ";
	$sql16 = "select count(*) as renew_year_count from " . T_LOG_OPEN_VIP . " where {$where} AND isYear=1 AND yellowDiamondYear=1 ";
