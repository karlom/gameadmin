...
if ( $action == 'add' || $action == 'update' ) {
	...
	$check = array();
	$checkSQL = "SELECT * FROM t_activity WHERE state=0 AND type='rank'";
...
// activity on live;
$live = array();
$liveSQL = "SELECT * FROM t_activity WHERE state=0 AND type='rank'";
$live = GFetchRowSet($liveSQL);

// activity history;
$history = array();
$historySQL = "SELECT * FROM t_activity WHERE state=1 AND type='rank'";
...
$smarty -> assign($rs);
$smarty -> display('module/activity/rankEdit.tpl');
		...
		for ( $i=0; $i<count($live); $i++) {
			...
			for($j=0; $j<count($prizes); $j++){
				...
				if ($live[$i]['type'] == 'rank'){
					...
					$prizes[$j][items][0]= array($prizes[$j]['collectEquip'],$prizes[$j]['collectNum']);
			...
			for($j=0; $j<count($arr[$i]['conditionAward']); $j++){
				for($k=0; $k<count($arr[$i]['conditionAward'][$j]); $k++){
					for($m=0; $m<count($arr[$i]['conditionAward'][$j][$k]); $m++){
						$arr[$i]['conditionAward'][$j][$k] = array_map('intval', $arr[$i]['conditionAward'][$j][$k]);
					};
				};
			};
		};
