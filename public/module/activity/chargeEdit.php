...
if ( $action == 'add' || $action == 'update' ) {
	...
	$data['cmd'] =  $_POST['subActivity'];
	$data['arg'] = '';
	$data['conditionAward'] = SS(json_encode($_POST['prizes']));
//echo $data['conditionAward'];die();
	...
	$check = array();
	$checkSQL = "SELECT * FROM t_activity WHERE state=0 AND type='charge'";
...
// activity on live;
$live = array();
$liveSQL = "SELECT * FROM t_activity WHERE state=0 AND type='charge'";
$live = GFetchRowSet($liveSQL);

// activity history;
$history = array();
$historySQL = "SELECT * FROM t_activity WHERE state=1 AND type='charge'";
...
$smarty -> assign($rs);
$smarty -> display('module/activity/chargeEdit.tpl');
