...
$smarty->assign( 'bodyLevel', $bodyLevel );
$smarty->assign( 'bodyDef', $bodyDef );
$smarty->display( 'module/player/body_upgrade.tpl' );
...
function getRecords($roleName, $accountName, $startTimestamp, $endTimestamp, $page, $pageSize){
    ...
    $sql = "select count(id) record_num from ".T_LOG_BODY_UPGRADE." where {$where}";
    $result = GFetchRowOne($sql);
    $data['recordCount'] = $result['record_num'];
    $limit = " LIMIT ".($page - 1) * $pageSize.",".$pageSize;
    $sql = "select * from ".T_LOG_BODY_UPGRADE." where {$where} order by id desc {$limit}";
