...
$smarty->display( 'module/player/vein_root_upgrade.tpl' );
...
function getRecords($roleName, $accountName, $startTimestamp, $endTimestamp, $page, $pageSize){
    ...
    $sql = "select count(id) record_num from ".T_LOG_VEIN_ROOT_UPGRADE." where {$where}";
    $result = GFetchRowOne($sql);
    $data['recordCount'] = $result['record_num'];
    $limit = " LIMIT ".($page - 1) * $pageSize.",".$pageSize;
    $sql = "select * from ".T_LOG_VEIN_ROOT_UPGRADE." where {$where} order by id desc {$limit}";
