...
$viewData = getLoginHistoryData($where,$startNum,$record,$counts);
...
$smarty->assign ($data);
$smarty->display ( 'module/player/login_history.tpl' );

function getLoginHistoryData($where,$startNum,$record,& $counts) {
    $sql = "SELECT * FROM ".T_LOG_LOGIN." WHERE {$where} ORDER BY mtime DESC limit {$startNum},{$record}";
    $rs = GFetchRowSet($sql);

    $sqlCount = "select count(*) as cnt from ".T_LOG_LOGIN." where {$where} ";
