...
$smarty->display( 'module/player/player_mail.tpl' );
...
function getDealRecords($roleName = '', $accountName = '', $startTimestamp, $endTimestamp, $page, $pageSize)
{
	...
	if( Validator::stringNotEmpty($roleName) && Validator::stringNotEmpty($accountName) )
	{
		$andCond[] = " ( ( sender_role_name = '$roleName' AND sender_account_name = '$accountName' ) OR ( receiver_role_name = '$roleName' AND receiver_account_name = '$accountName' ) ) ";
	} 
	elseif( Validator::stringNotEmpty($roleName) ) 
	{
		$andCond[] = " sender_role_name = '$roleName' OR receiver_role_name = '$roleName' ";
	}
	elseif( Validator::stringNotEmpty($accountName) ) 
	{
		$andCond[] = " sender_account_name = '$accountName' OR receiver_account_name = '$accountName' ";
	}
...
//	$orderBy = ' ORDER BY ' .  $orderBy[0] . ' ' . $orderBy[1] . ' ';
	...
	$sql = 'SELECT * FROM ' . T_LOG_MAIL  . $whereCond . $orderBy . $limit;
	...
	{
		if( Validator::stringNotEmpty($record['items']) )
		{
			// 解析附件的物品
			$record['self_item_list'] = array();
			$items = explode( ',', $record['items'] );
			foreach( $items as $item )
			{
				if(Validator::stringNotEmpty($item))
				{
					list($item_id, $item_count, $item_attr_str) = explode('#', $item);
					$record['item_list'][$item_id] = getItemDetailArray($item_attr_str);
					$record['item_list'][$item_id]['item_count'] = $item_count;
				}
			}
		}
	}

	$sqlCount = 'SELECT COUNT(*) row_count FROM ' . T_LOG_MAIL . $whereCond ;
