...
{
	$familyName 		= SS( $_REQUEST['familyName'] );
}
...
//家族名
if( !empty($familyName) ) {
	$andCondArray[] = " familyName = '{$familyName}' ";
}
...
{  
	...
	// 获取满足条件的记录
	$sql 	= "SELECT 
					account_name, role_name, familyName, content, mtime
				FROM " . T_LOG_FAMILY_CHAT . " 
				WHERE 
					$cond 
				ORDER BY 
					mtime DESC";
...
$smarty->display( 'module/msg/family_chat_log.tpl' );