...
foreach ($viewData as $ke => &$va) {
	if($va['toned_color'] == 2) {
		$va['toned_color'] = $lang->unmount->blue;
	}

	if($va['toned_color'] == 3) {
		$va['toned_color'] = $lang->unmount->purple;
	}

	if($va['toned_color'] == 4) {
		$va['toned_color'] = $lang->unmount->orange;
	}

	if($va['toned_color'] == 5) {
		$va['toned_color'] = $lang->unmount->gold;
	}
}
...
$smarty->display("module/basedata/equip_unmount_check.tpl");
exit;
...
function getDataRecord($tmp,$where='',$order='',$startNum='',$record='',&$counts='', $filter = false){
    ...
    if( $filter )
    {
		...
		$sql = " select t1.`mtime`,t1.`account_name`,t1.`role_name`,t1.`level`,t1.`equip_uid`,t1.`equip_id`,t1.`stoned_id`,t1.`stoned_color`,t1.`stoned_level`,t1.`unmount_id`,t1.`unmount_num`,t1.`equip_success_rate`,t1.`equip_rate`,t1.`stone_success_rate`,t1.`stone_rate`,t1.`cost`,t1.`stone_safe`,t1.`equip_safe`,t1.`result` from `".T_LOG_UNMOUNT."` t1 $join where {$where} $whereCond"; 
		if(!empty($order)) {
			$sql .= " {$order} ";
		}
		if($tmp==1) {
		...
		$counts = GFetchRowOne("SELECT COUNT(t1.`account_name`) as counts FROM `".T_LOG_UNMOUNT."` t1 $join  where {$where} $whereCond");
