...
$smarty->display("module/basedata/equip_make_check.tpl");
exit;
...
function getCopySceneRecord($tmp,$where='',$order='',$startNum='',$record='',&$counts='', $filter = false){
    ...
    if( $filter )
    {
		...
		$sql = " select t1.`mtime`,t1.`account_name`,t1.`role_name`,t1.`level`,t1.`material_one`,t1.`material_count_one`,t1.`material_two`,t1.`material_count_two`,t1.`material_three`,t1.`material_count_three`,t1.`material_four`,t1.`material_count_four`,t1.`material_five`,t1.`material_count_five`,t1.`material_six`,t1.`material_count_six`,t1.`product_uid`,t1.`product`,t1.`product_count`,t1.`cost` from `".T_LOG_MAKE."` t1 $join where {$where} $whereCond"; 
		if(!empty($order)) {
			$sql .= " {$order} ";
		}
		if($tmp==1) {
		...
		$counts = GFetchRowOne("SELECT COUNT(t1.`account_name`) as counts FROM `".T_LOG_MAKE."` t1 $join where {$where} $whereCond");
