...
$attr = array('power','phy','energy','steps','atk','def','maxHp','maxMp');
foreach ($viewData as $key => &$value){
	$value['old']     = array_combine($attr,explode('-',$value['equip_old_attr']));
	$value['oldStar'] = array_combine($attr,explode('-',$value['equip_old_attr_star']));
	$value['new']     = array_combine($attr,explode('-',$value['equip_new_attr']));
	$value['newStar'] = array_combine($attr,explode('-',$value['equip_new_attr_star']));
}
...
$smarty->display("module/basedata/equip_wash_check.tpl");
exit;
...
function getCopySceneRecord($tmp,$where='',$order='',$startNum='',$record='',&$counts='', $filter = false){
	...
	$sql = " select t1.`mtime`,t1.`account_name`,t1.`role_name`,t1.`level`,t1.`equip_uid`,t1.`equip_id`,t1.`equip_color`,t1.`stone_id`,t1.`equip_old_attr`,t1.`equip_old_attr_star`,t1.`equip_new_attr`,t1.`equip_new_attr_star`,t1.`cost` from `".T_LOG_WASH."` t1 $join where {$where} $whereCond"; 
	if(!empty($order)) {
		$sql .= " {$order} ";
	}
	if($tmp==1) {
...
//	echo $sql;
	...
	$counts = GFetchRowOne("SELECT COUNT(t1.`account_name`) as counts FROM `".T_LOG_WASH."` t1 $join where {$where} $whereCond");
