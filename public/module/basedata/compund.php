...
$smarty->display("module/basedata/compund.tpl");
exit;
...
function getDataRecord($tmp,$where='',$order='',$startNum='',$record='',&$counts='', $filter = false){
    ...
    if( $filter )
    {
		...
		$sql = " select t1.mtime,t1.role_name,t1.account_name,t1.level,t1.materal_id,t1.materal_count,t1.symbol_id,t1.rate,t1.actual_rate,t1.money_cost,t1.compound_id,t1.success from `t_log_compound` t1 $join where {$where} $whereCond"; 
		if(!empty($order)) {
			$sql .= " {$order} ";
		}
		if($tmp==1) {
		...
		$counts = GFetchRowOne("SELECT COUNT(t1.`account_name`) as counts FROM `t_log_compound` t1 $join where {$where} $whereCond");
