 ...
 /*
	...
	for($i = 1; $i <= $itemCount; $i++)
	{
		...
		$cheapShopList[$i] = array(
								...
								'limitNum' => intval($_GET['buy_limit_num'][$i]),
							);
...
//	{
		...
		$ret = interfaceRequest('setcheapshop', array('refineshop' => json_encode( $cheapShopList ), 'type' => 2 ));
		if($ret['result'] == 1)
		{
...
//	dump($_POST);
    ...
    $id = intval($_GET['id']);
    $sql = "select * from ".T_REFINE_SHOP." where id={$id}";
	...
	foreach($itemId as $key => $value){
		if($value){
			...
			$itemArr['limitNum'] = intval($limitNum[$key]);
	...
	if(!$inValid)
	{
		if($id){
			$cheapShopShow['id'] = $id;
			$sql = makeUpdateSqlFromArray($cheapShopShow, T_REFINE_SHOP);
		}else{
			$sql = makeInsertSqlFromArray($cheapShopShow, T_REFINE_SHOP);
		}
		GQuery($sql);
	}else{
		$msg[] = "日期和其它配置(#$existID)重叠，请重设！";
    ...
    $id = intval($_GET['id']);
    $sql = "delete from ".T_REFINE_SHOP." where id={$id}";
    GQuery($sql);
	...
	if(isset($profile['id']) && Validator::stringNotEmpty($profile['content']))
	{
		...
		$ret = interfaceRequest('setcheapshop', array('refineshop' => json_encode($params), 'type' => 2 ));
		...
		if($ret['result'] == 1)
		{
			...
			// 把当前进行的配置改成已结束
			$sql = "UPDATE ".T_REFINE_SHOP." SET status = 2 WHERE status = 1";
			GQuery($sql);
			
			// 更新当前配置状态为进行中
			$sql = "UPDATE ".T_REFINE_SHOP." SET status = 1 WHERE id = " . $id;
			GQuery($sql);
...
$sql = "select * from ".T_REFINE_SHOP." order by end_time desc,id desc";
...
$smarty->display('module/basedataconfig/refine_shop.tpl');
...
function getCheapShopInfo( $forceRemote = false )
{
	...
	{// 缓存不存在，从远程获取
		...
		$ret = interfaceRequest($api, array('type' => 2));
		$cheapShopInfo = array();
		if( $ret['result'] == 1 )
		{
			$cheapShopInfo = $ret;
